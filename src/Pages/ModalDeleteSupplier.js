...
class ModalDeleteSupplier extends React.Component {
  state = {
    open: false,
  };
  onOpenModal = () => {
  ...
  handleDelete = event => {
    ...
    const url = "https://ec2-54-179-157-83.ap-southeast-1.compute.amazonaws.com/api/users/suppliers/";
    const headers = {
      Authorization: "Bearer " + token
    };

      this.onCloseModal()
      this.props.DeleteSuppliers(url,this.props.name,headers).then( ()=> {
...
export default connect("toggle, from, to, is_login, token", actions)(withRouter(ModalDeleteSupplier))