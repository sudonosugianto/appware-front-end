...
class ModalDeletePackage extends React.Component {
  state = {
    open: false,
  };
  onOpenModal = () => {
  ...
  handleDelete = event => {
    ...
    const url = "http://ec2-54-179-157-83.ap-southeast-1.compute.amazonaws.com:5000/api/users/packages/";
    const headers = {
      Authorization: "Bearer " + token
    };
        this.props.DeletePackages(url,this.props.name, headers).then( ()=> {
...
export default connect("toggle, from, to, is_login, token", actions)(withRouter(ModalDeletePackage))